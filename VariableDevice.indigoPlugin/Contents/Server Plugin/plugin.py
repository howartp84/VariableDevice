...
class Plugin(indigo.PluginBase):
	...
	def deviceStartComm(self, dev):
		...
		for var in varsList:
			...
			if (int(varID) == int(state)):
				self.debugLog("Updating displayState: {}".format(varValue))
	...
	def deviceStopComm(self, dev):
		...
		for var in varsList:
			varID = int(var)
			indigo.server.log("Stop monitoring variable: {}".format(indigo.variables[varID].name))
			try:
	...
	def variableUpdated (self, origVar, newVar):
		if (newVar.id in self.watchIDs):
			...
			#self.debugLog(devID)
			self.debugLog("Update received: {}: {}".format(newVar.name, newVar.value))
			#self.debugLog("Variable is monitored by {}".format(indigo.devices[devID].name))
